...
Hd = 0*4*pi*Ms/Hc;             % Demagnetizing field
Kd = Ms*Hd*Hc / 2;
KdV = Kd *V;
...
ABS_m_cross_dmdt = sum(m_cross_dmdt(3,:)*dt)
% ----------------------------------------------
tau = t;
dtau = tau(2) - tau(1);
t = t*tau_c*1e9;
...
ABS_ratio_slz = abs(Nslz_provided / Ns)%(Ns*(mz(1)-mz(end)))
sl_Over_slz = ratio_sl/ABS_ratio_slz;
